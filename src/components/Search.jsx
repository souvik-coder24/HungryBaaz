...
export default function Search({ latitude, longitude }) {
    ...
    return (
        <div className='mt-16 w-full ml-[350px] mx-auto'>
            <div className="relative">
                <input
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className='border-2 bg-gray-300 border-gray-400 py-3 focus:outline-none rounded-md w-[60%]'
                    type="text"
            ...
            <div className="my-7 flex gap-3">
                {filterBtn.map((btn) => (
                    <button
                        ...
                        className={`filterBtn flex gap-2 items-center px-4 py-2 rounded-md ${activeBtn === btn.filtername ? "bg-gray-500 text-white" : "bg-gray-200 text-gray-700"}`}
                    >
                        {btn.filtername}
                    </button>
                ))}
            ...
            <div className="relative">
                <div className="flex flex-wrap w-[60%] gap-6">
                    {/* Dishes */}
                    {activeBtn === "Dishes" && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {dishes.length > 0 ? (
                                    dishes.map((item, index) => {
                                        ...
                                        return (
                                            <div key={index} className="bg-white p-4 rounded-md shadow-2xl">
                                                    ...
                                                    <div className="flex justify-between">
                                                        <div>
                                                            ...
                                                            <div className="flex items-center mb-2">
                                                                ...
                                                                <p className="text-gray-600 ml-1 font-bold">{itemInfo?.info?.ratings?.aggregatedRating?.rating}</p>
                                                                <p className="ml-1 text-gray-500 font-bold">({itemInfo?.info?.ratings?.aggregatedRating?.ratingCountV2})</p>
                                                            </div>
                                ...
                                {restroData.length > 0 ? (
                                    restroData.map((item, index) => {
                                        ...
                                        return (
                                            ...
                                            <div key={index} className="flex bg-white p-4 rounded-md shadow-2xl transition-transform duration-500 ease-in-out transform hover:scale-95 cursor-pointer">
                                                <div className="flex-grow">
